...
def distance_to_angle_left(distance):
    if distance is None:
        return 90
    return 90 + ((200 - min(200, distance)) / 200) * 30
...
if __name__ == '__main__':
    ...
    try:
        while not stop_robot_flag:
            ...
            # Wenn der Roboter die Richtung noch nicht erkannt hat, müssen wir die erste Linie finden
            if Richtung is None:
                if distance <= 50 and Richtung is None:
                    if distanceL < distanceR:
                        Richtung = "Rechts"
                        print("Richtung erkannt: Rechts")
                        LenkungGerade()
                        Antrieb.Motor(2, 1, speed_set)
                        time.sleep(0.6)
                    elif distanceR < distanceL:
                        Richtung = "Links"
                        print("Richtung erkannt: Links")
                        LenkungGerade()
                        Antrieb.Motor(2, 1, speed_set)
                        time.sleep(0.6)
